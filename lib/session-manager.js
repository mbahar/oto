 ...
 * 
 * Integrated with:
 * - Session health checks (session-validator.js)
 * - MFA/2FA support (mfa-handler.js)
 * 
 ...
 * 
 * Options:
 * - checkHealth: boolean (default true) - validate session before launching
 * - autoRefresh: boolean (default false) - attempt to refresh expired sessions
 * - requireValid: boolean (default false) - throw error if session invalid
 */
async function launchSession(platform, account = 'default', headless = true, options = {}) {
  const {
    checkHealth = true,
    autoRefresh = false,
    requireValid = false
  ...
  // Check session health if requested
  if (checkHealth && hasExisting) {
    const health = validator.isSessionValid(platform, account);
    
    if (!health.valid) {
      console.log(`⚠️  Session health check failed: ${health.reason}`);
      
      if (requireValid) {
        throw new Error(`Session invalid: ${health.reason}`);
      }
      
      if (!autoRefresh) {
        console.log(`   To refresh, run: node scripts/save-session.js ${platform} <url> ${account}`);
      }
    }
  }
  ...
  return {
    ...
    // Check if session is still valid (e.g., after navigation)
    checkHealth: async () => {
      const expiry = await validator.detectSessionExpiry(page, platform);
      if (expiry.expired) {
        validator.markSessionStale(platform, account, expiry.indicator);
        return { valid: false, reason: expiry.indicator };
      }
      return { valid: true };
    },
    // Try to auto-refresh if session appears expired
    autoRefresh: async () => validator.autoRefreshSession(page, platform, account),
    // Detect and handle MFA if needed
    detectMFA: async () => mfa.detectMFAPrompt(page),
  };
}
...
module.exports = {
  ...
  sessionPath,
  // Validators
  isSessionValid: validator.isSessionValid,
  getSessionAge: validator.getSessionAge,
  getAllSessionHealth: validator.getAllSessionHealth,
  // MFA
  detectMFAPrompt: mfa.detectMFAPrompt,
  getMFASettings: mfa.getMFASettings,
  listMFASessions: mfa.listMFASessions,
};
